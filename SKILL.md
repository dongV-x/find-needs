...
只有当前 Agent 已成功加载本 Skill 时，才开始正式访谈。场景任务单只提供项目背景、当前系统和本轮范围，不能复制或替代本 Skill 的流程。

若任务单要求使用本 Skill，但当前环境尚未安装或加载，先按任务单提供的安装地址自行安装；安装失败、当前 Agent 不支持 Skills，或安装后当前会话仍无法加载时，停止在访谈开始前，请用户联系任务负责人或 AI 开发协助。不得在未加载 Skill 时根据任务单自行拼凑一套降级访谈。
